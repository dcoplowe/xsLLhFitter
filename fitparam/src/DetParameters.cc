...
//ctor
DetParameters::DetParameters(const char *fname,TVectorD* detweights, std::vector<AnaSample*> samples, const char *name)
{
    m_name    = name;
    ...
    //DC 02/06/16: Can build event map in the constructor as we now feed in the samples to the constructor.
    InitEventMap(samples);
...
// --
int DetParameters::GetBinIndex(double D1, double D2, int sample_id)
{
    int binn = BADBIN;
    for(size_t i=0;i<m_bins.size();i++)
    {
...
void DetParameters::InitEventMap(std::vector<AnaSample*> &sample)
{
    ...
    for(size_t s=0;s<sample.size();s++)
    {
        ...
        for(int i=0;i<sample[s]->GetN();i++)
        {
            ...
            double D1 = ev->GetRecD1trk();
            double D2 = ev->GetRecD2trk();
            
            cout << "D1 " << D1 << " D2 " << D2 << endl;
            
            int reaction = ev->GetReaction();
            int binn   = GetBinIndex(D1, D2,sample[s]->GetSampleType());
            if(binn == BADBIN)
            {
