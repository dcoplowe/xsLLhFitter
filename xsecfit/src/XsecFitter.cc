...
void XsecFitter::FixParameter(string par_name, double value){
    for(int i=0;i<m_npar;i++)
    {
        ...
        if(pos != std::string::npos){
            fitter->SetParameter(i, par_names[i].c_str(), value, 0, value, value);
...
void XsecFitter::fcn(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag)
{
    ...
    if(xsfVerbose){
        ...
        cout << "Chi2 stat / syst: " << chi2_stat << " / " << chi2_sys << endl;
    }
...
void XsecFitter::DoSaveResults(vector< vector<double> > parresults, vector< vector<double> > parerrors, double chi2){
    ...
    //DC: Added 13/05/16
    iterations->SetBinContent(1,m_calls);
